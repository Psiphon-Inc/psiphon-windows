...
{
    printf("PoTTY Link: command-line connection utility\n");
    printf("%s\n", ver);
    printf("Usage: plonk [options] [user@]host [command]\n");
    ...
    printf("  -noagent  disable use of Pogeant\n");
    printf("  -agent    enable use of Pogeant\n");
...
int main(int argc, char **argv)
{
	...
	if (*p == '-') {
	    ...
	    if (ret == -2) {
		fprintf(stderr,
			"plonk: option \"%s\" requires an argument\n", p);
		errors = 1;
	    } else if (ret == 2) {
		--argc, ++argv;
	    } else if (ret == 1) {
		...
		fprintf(stderr, "plonk: unknown option \"%s\"\n", p);
		errors = 1;
	    }
	} else if (*p) {
	...
	fprintf(stderr, "Plonk requires WinSock 2\n");
	return 1;
    }
