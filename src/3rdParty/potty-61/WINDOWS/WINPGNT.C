...
void modalfatalbox(char *fmt, ...)
{
    ...
    va_end(ap);
    MessageBox(hwnd, buf, "Pogeant Fatal Error",
...
{
    static const char mbtitle[] = "PoTTY Key File Warning";
    static const char message[] =
	"You are loading an SSH-2 private key which has an\n"
	"old version of the file format. This means your key\n"
	"file is not fully tamperproof. Future versions of\n"
	"PoTTY may stop supporting this private key format,\n"
	"so we recommend you convert your key to the new\n"
	"format.\n"
	"\n"
	"You can perform this conversion by loading the key\n"
	"into PoTTYgen and then saving it again.";
...
static void add_keyfile(Filename filename)
{
	...
	if (already_running) {
		...
		MessageBox(NULL, "The already running Pogeant "
...
static BOOL AddTrayIcon(HWND hwnd)
{
    ...
    strcpy(tnid.szTip, "Pogeant (PoTTY authentication agent)");
	...
	switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
	  case IDM_PUTTY:
	    if((int)ShellExecute(hwnd, NULL, putty_path, _T(""), _T(""),
				 SW_SHOW) <= 32) {
		MessageBox(NULL, "Unable to execute PoTTY!",
	  ...
	  default:
		...
		if(wParam >= IDM_SESSIONS_BASE && wParam <= IDM_SESSIONS_MAX) {
		    ...
		    if((int)ShellExecute(hwnd, NULL, putty_path, param,
					 _T(""), SW_SHOW) <= 32) {
			MessageBox(NULL, "Unable to execute PoTTY!", "Error",
				   MB_OK | MB_ICONERROR);
		    }
		}
	    }
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	 ...
	 */
        if (!init_advapi()) {
	    MessageBox(NULL,
		       "Unable to access security APIs. Pogeant will\n"
		       "not run, in case it causes a security breach.",
		       "Pogeant Fatal Error", MB_ICONERROR | MB_OK);
	    return 1;
	}
#else
	MessageBox(NULL,
	...
	if (!command && !added_keys) {
	    MessageBox(NULL, "Pogeant is already running", "Pogeant Error",
		       MB_ICONERROR | MB_OK);
	}
	if (advapi)
	    FreeLibrary(advapi);
